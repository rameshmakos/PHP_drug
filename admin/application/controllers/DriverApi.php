...
//		mail ('ramesh.makos@gmail.com',$subject,$body,$header);
if( mail ('ramesh.makos@gmail.com',$subject,$body,$header) )
    ...
    public function update_driver_location(){
            ...
            $_POST = $_REQUEST; 
                ...
                $this->form_validation->set_rules('location_lat', 'lat', 'trim|required');
                $this->form_validation->set_rules('location_lng', 'lng', 'trim|required');
...
//echo $this->db->last_query();exit();
                    ...
                    $data["code"] = 2;  
                    $data["msg"] = "Location not updated successfully.";
        			$data["details"]=array();  
                ...
                }else
                {
                    ...
                    if(isset($_FILES["signature"]) && $_FILES["signature"]["size"] > 0){
                        ...
                        else
                        {
                            ...
                            $add["signature"]=$file_data['file_name'];//
                    ...
                    $this->load->model("common_model");
                    $q =$this->common_model->data_update("sale",$add,array("sale_id"=>$_REQUEST["sale_id"]));
                    
                    if($q){
                        
                        
                        $data["code"] = 1; 
                        $data["msg"] = 'signature saved successfully';
                    }
                    else{
                        $data["code"] = 2; 
                        $data["msg"] = 'signature not saved successfully';
                    }
                }
...
public function order_details()
{
    ...
    public function cancel_order(){
        		...
        		}else
                {
                    ...
                    $check=$this->db->query("Update sale set delivery_datetime=CURRENT_TIMESTAMP, status = 3 where user_id = '".$this->input->post("user_id")."' and  sale_id = '".$this->input->post("sale_id")."' ");    
                    if($check)
                    {
    ...
    public function delivered_order_complete(){
        		...
        		}else
                {
                    ...
                    $details=$details1->row();
                    $order=$details1->row();
                    $check1=$this->db->query("Update sale set delivery_datetime=CURRENT_TIMESTAMP,status = 4 where user_id = '".$this->input->post("user_id")."' and  sale_id = '".$this->input->post("sale_id")."' ");    
                    if($check1)
                    {
                $check2=$this->db->query("update sale set delivery_datetime=CURRENT_TIMESTAMP,status = 4 where sale_id = '".$this->input->post("sale_id")."'");
...
where sale_id = '".$this->input->post("sale_id")."'"); 
    ...
    public function confirm_order(){
        		...
        		}else
                {
				    ...
				    $details1=$this->db->query("SELECT * FROM sale where sale_id = '".$this->input->post("sale_id")."' "); 
                    $details=$details1->row();
                    $check=$this->db->query("Update sale set delivery_datetime=CURRENT_TIMESTAMP,status = 1 where user_id = '".$this->input->post("user_id")."' and  sale_id = '".$this->input->post("sale_id")."' ");    
                    if($check)
                    {
                            $user = $q->row();  
     ...
     public function out_order(){
        		...
        		}else
                {
				    ...
				    $details1=$this->db->query("SELECT * FROM sale where sale_id = '".$this->input->post("sale_id")."' "); 
                    $details=$details1->row();
                    $check=$this->db->query("Update sale set delivery_datetime=CURRENT_TIMESTAMP,status = 2 where user_id = '".$this->input->post("user_id")."' and  sale_id = '".$this->input->post("sale_id")."' ");    
                    if($check)
                    {
                            $user = $q->row();  
                            $message["title"] = "Out for delivery order";
                            $message["body"] = "Out for Delivery order";//"Your order Number '".$this->input->post("sale_id")."' Delivered successfully. Thank you for being with us";
                            $message["image"] = "";
                            $message["created_at"] = date("Y-m-d h:i:s"); 
                            $message["obj"] = "";
...
/* about us */
 ...
 /* end about us */         
    ...
    $this->load->helper('gcm_helper');
    $gcm = new GCM();   
    $result = $gcm->send_notification(array("AAAAgv35W5M:APA91bE4Q6EArUAWlBiJwV4GoHwnPmzj0aLf8aUSJDjXrFLHU3B0Jh9YSSbfUCIB2d33xNSDHh7LsY3BibyNkKE3cQfW0GjLuboEjtYQME3YAtJaSucbHWAsyJdGovRhWsf6TfbQB2I-"),$message ,"android",false);
...
public function getTaskByDate(){
        ...
        public function mark_delivered_old()
        {   
